...
double RK_4(double (*f)(double, double, double), double (*g)(double, double, double), double x_0, double y_0, double z_0, double h, string filename, double xmin, double xmax){
	...
	for(x = x_0/h; x > xmin/h; x -= 1){				    //changing x to x/h to avoid floating increment
		...
		k2y = h1*f(x*h+h1/2, y + k1y/2, z + k1z/2);
		k2z = h1*g(x*h+h1/2, y + k1y/2, z + k1z/2);		
		
		k3y = h1*f(x*h+h1/2, y + k2y/2, z + k2z/2);
		k3z = h1*g(x*h+h1/2, y + k2y/2, z + k2z/2);

		k4y = h1*f(x*h+h1, y + k3y, z + k3z);
		k4z = h1*g(x*h+h1, y + k3y, z + k3z);
    ...
    */
	...
	for(x = x_0/h; x < xmax/h; x += 1){			    //changing x to x/h to avoid floating increment
		...
		k2y = h*f(x*h+h/2, y + k1y/2, z + k1z/2);
		k2z = h*g(x*h+h/2, y + k1y/2, z + k1z/2);		
		
		k3y = h*f(x*h+h/2, y + k2y/2, z + k2z/2);
		k3z = h*g(x*h+h/2, y + k2y/2, z + k2z/2);

		k4y = h*f(x*h+h, y + k3y, z + k3z);
		k4z = h*g(x*h+h, y + k3y, z + k3z);
...
void check_shooting(double (*f)(double, double, double), double (*g)(double, double, double), double x_0, double y_0,double x_1, double y_1, double slope, double dx, string filename){
	double y = RK_4(f,g,x_0,y_0,slope,dx,filename,x_0,x_1);
    cout<<y<<endl;
    cout<<y_1<<endl;
	if(abs(y - y_1) < 1e-6){					//error accepted 0.001
...
void shooting(double (*f)(double, double, double), double (*g)(double, double, double), double x_0, double y_0, double x_1, double y_1, double lb, double ub, double dx, string filename){
										                    //find yl and yu
	double yl = RK_4(f,g,x_0,y_0,lb,dx,filename,x_0,x_1);
	double yu = RK_4(f,g,x_0,y_0,ub,dx,filename,x_0,x_1);

	double yc = yl;
	if(abs(y_1 - yl) <= 1e-6){					    	//for any boundary value as solution
		RK_4(f,g,x_0,y_0,lb,dx,filename,x_0,x_1);
	}
	else if(abs(y_1 - yu) <= 1e-6){
		RK_4(f,g,x_0,y_0,ub,dx,filename,x_0,x_1);
	}
	while(abs(yc - y_1) >= 1e-6 ){				    	    //loop for interpolation
			double c;
			if((yl < y_1)&&(yu > y_1)){
